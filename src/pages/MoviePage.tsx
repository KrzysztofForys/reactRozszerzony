...
export default function MoviePage() {
  ...
  const { data: movie, isLoading } = useQuery({
    ...
    queryFn: () => moviesApi.details(movieId),
    enabled: !!movieId
  });
  ...
  return (
    <article>
      ...
      <div className="flex gap-8 mb-10 flex-wrap">
        <img
          src={tmdbImage(movie.poster_path, 'w342') ?? undefined}
          alt={movie.title}
          ...
          <div className="grid grid-cols-4 lg:grid-cols-6 gap-4 mb-10">
            {movie.credits.cast.slice(0, 12).map((person: CastMember) => (
              <div key={person.id}>
                {person.profile_path ? (
                  <img
                    src={tmdbImage(person.profile_path, 'w185') ?? undefined}
                    alt={person.name}
      ...
      {movie.similar?.results?.length > 0 && (
        <>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4">
            {movie.similar.results.slice(0, 12).map((m: Movie) => (
